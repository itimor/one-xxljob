...
<script>
  import {mailbot, auth} from '@/api/all'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        mailbot.requestGet(this.listQuery).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.loading = true
            mailbot.requestPost(this.temp).then(response => {
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.loading = true
            mailbot.requestPut(this.temp.id, this.temp).then(() => {
              this.dialogFormVisible = false
              this.$notify({
                title: '成功',
                message: '更新成功',
      ...
      handleDelete(row) {
        this.$confirm('是否确定删除?', '提示', {
          ...
          mailbot.requestDelete(row.id).then(() => {
            this.$message({
              message: '删除成功',
              type: 'success'
            })
      ...
      handleBatchDel() {
        ...
        this.$confirm('是否确定删除?', '提示', {
          ...
          mailbot.requestBulkDelete(ids).then(response => {
